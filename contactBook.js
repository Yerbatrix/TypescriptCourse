var addContact = function (addressBook, contact) {
    if (!contact.name || !contact.email || !contact.phone) {
        console.log("Błąd: Wszystkie pola (imię, email, telefon) muszą być wypełnione.");
        return;
    }
...
var getContacts = function (addressBook) {
    ...
    addressBook.forEach(function (contact, index) {
        console.log("".concat(index + 1, ". ").concat(contact.name, " - ").concat(contact.email, " - ").concat(contact.phone));
    });
};
var findContactByName = function (addressBook, name) {
    var contact = addressBook.find(function (contact) { return contact.name.toLowerCase() === name.toLowerCase(); });
    if (contact) {
        console.log("Znaleziono kontakt: ".concat(contact.name, " - ").concat(contact.email, " - ").concat(contact.phone));
    }
    else {
...
var removeContact = function (addressBook, name) {
    var index = addressBook.findIndex(function (contact) { return contact.name.toLocaleLowerCase() === name.toLocaleLowerCase(); });
    if (index !== -1) {
        var removedContact = addressBook.splice(index, 1);
        console.log("Usuni\u0119to kontakt: ".concat(removedContact[0].name));
    }
    else {
        console.log("Nie znaleziono kontaktu o imieniu: ".concat(name, ", nie mo\u017Cna usun\u0105\u0107."));
    }
};
var addressBook = [];
