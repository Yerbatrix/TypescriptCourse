"use strict";
const addContact = (addressBook, contact) => {
...
const getContacts = (addressBook) => {
    console.log("\nlista kontaktów:");
    addressBook.forEach((contact, index) => {
        console.log(`${index + 1}. ${contact.name} - ${contact.email} - ${contact.phone}`);
    });
};
const findContactByName = (addressBook, name) => {
    const contact = addressBook.find((contact) => contact.name.toLowerCase() === name.toLowerCase());
    if (contact) {
        console.log(`Znaleziono kontakt: ${contact.name} - ${contact.email} - ${contact.phone}`);
    }
    else {
        console.log(`Nie znaleziono kontaktu o imieniu: ${name}.`);
    }
};
const removeContact = (addressBook, name) => {
    const index = addressBook.findIndex((contact) => contact.name.toLocaleLowerCase() === name.toLocaleLowerCase());
    if (index !== -1) {
        const removedContact = addressBook.splice(index, 1);
        console.log(`Usunięto kontakt: ${removedContact[0].name}`);
    }
    else {
        console.log(`Nie znaleziono kontaktu o imieniu: ${name}, nie można usunąć.`);
    }
};
const addressBook = [];
